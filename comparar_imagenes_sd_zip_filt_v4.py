...
#st.title("AGEAI: Imágenes y Metadatos")
...
if uploaded_file:
    ...
    if 'NEUTRAL' in folders and 'OLDER' in folders:
        ...
        # Almacenar los metadatos de cada imagen
        metadata_dict = {}
        for image_name in common_images:
            metadata1 = get_png_metadata(images1[image_name])
            metadata2 = get_png_metadata(images2[image_name])
            metadata_dict[image_name] = (metadata1, metadata2)

        # Campo de entrada para filtrar imágenes por prompt
        search_query = st.text_input("Filtrar por prompt:")

        # Filtrar las imágenes según el término de búsqueda
        filtered_images = []
        for image_name, (metadata1, metadata2) in metadata_dict.items():
            prompt1 = metadata1.get('parameters', '')
            prompt2 = metadata2.get('parameters', '')
            if search_query.lower() in prompt1.lower() or search_query.lower() in prompt2.lower():
                filtered_images.append(image_name)

        for image_name in sorted(filtered_images):
            ...
            with col1:
                #st.header("NEUTRAL")
                st.markdown("<h3 style='text-align: center; color: white;'>Neutral</h1>", unsafe_allow_html=True)
            ...
            with col2:
                #st.header("OLDER")
                st.markdown("<h3 style='text-align: center; color: white;'>Older</h1>", unsafe_allow_html=True)
